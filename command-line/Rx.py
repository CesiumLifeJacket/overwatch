...
class SeqLengthMismatch(SchemaMismatch):
    def __init__(self, schema, data):
        ...
        message = 'sequence must have {} element{} (had {})'.format(
            expected_length,
            's'*(expected_length != 1),  # pluralization
            len(data)
            )
    ...
    def __init__(self, opt):
        if isinstance(opt, Range):
            ...
            if {'min', 'min-ex'}.issubset(opt):
                raise ValueError(
                    "Cannot define both exclusive and inclusive min"
                )

            if {'max', 'max-ex'}.issubset(opt):
                raise ValueError(
                    "Cannot define both exclusive and inclusive max"
                )
...
class Factory(object):
    ...
    def expand_uri(self, type_name):
        ...
        if not m:
            raise ValueError(
                "couldn't understand type name '{}'".format(type_name)
            )
        ...
        if prefix not in self.prefix_registry:
            raise KeyError(
                "unknown prefix '{0}' in type name '{}'".format(prefix,
                                                                type_name)
            )
    ...
    def add_prefix(self, name, base):
        if self.prefix_registry.get(name):
            raise SchemaError(
                "the prefix '{}' is already registered".format(name)
            )
...
class IntType(_CoreType):
    ...
    def __init__(self, schema, rx):
        ...
        self.value = None
        if 'value' in schema:
            if not isinstance(schema['value'], Number) or \
                    schema['value'] % 1 != 0:
    ...
    def validate(self, value):
        if not isinstance(value, Number) or isinstance(value, bool) or \
                value % 1 != 0:
            raise TypeMismatch(self, value)
...
class SeqType(_CoreType):
    ...
    def validate(self, value):
        ...
        for index, (schema, item) in \
                enumerate(zip(self.content_schema, value)):
            try:
